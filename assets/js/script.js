...
async function loadTimeSlotsWithAvailability(ymd) {
    ...
    if (serviceId == 0) {
        ...
        try {
            ...
            const allRes = await fetch("http://localhost/ClusterCode_GovConnect/backend/getTimeSlots.php");
            ...
            if (allResult.status === "success" && Array.isArray(allResult.data)) {
                ...
                const availableSlots = (availResult.status === "success" && Array.isArray(availResult.data))
                    ? availResult.data.map(s => s.id)
                    : [];

                let i = 0;
                // While loop to add options
                while (i < allSlots.length) {
                    ...
                    if (!availableSlots.includes(slot.id)) {
                        ...
                        option.textContent = `${slot.start_time} - ${slot.end_time}` + " (Already Received)";
                    }
    ...
    if (department == 0) {
        ...
        document.querySelectorAll("input[type='file']").forEach(input => {
            if (input.files.length > 0) {
                // append with the name attribute (documents[12], etc.)
        ...
        try {
            let response = await fetch("http://localhost/ClusterCode_GovConnect/backend/createAppointment.php", {
                method: "POST",
